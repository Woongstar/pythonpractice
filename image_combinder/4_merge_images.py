...
def add_file():
    ...
    filetypes=(("PNG 파일","*.png"),("모든파일","*.*")), initialdir=r"C:\Users\양지웅\PycharmProjects\pythonpractice")
    for file in files:
...
def browse_dest_path():
    folder_selected = filedialog.askdirectory(title="")
...
def merge_image():
    ...
    dest_path = os.path.join(txt_dest_path.get(),"nadophoto.jpg")
    result_img.save(dest_path)
    msgbox.showinfo("알림","작업이 완료되었습니다.")
def start():
    if list_file.size() == 0:
        msgbox.showwarning("겅고","이미지 파일을 추가하세요")
        return
